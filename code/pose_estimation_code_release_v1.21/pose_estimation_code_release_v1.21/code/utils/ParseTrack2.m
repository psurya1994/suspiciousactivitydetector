function [PM, CM, curPose, curSticks, curPM, angles, joints] = ParseTrack2(image1, motion, T, reparse, frixs, pars, verbose)
...
%    safe_imwrite(imPose, pose_fname);
  ...
  subplot(2,2,3), image(curSticks);
  subplot(2,2,4), image(imPose);
  ...
  pointXX=[];
  for i=1:num1
    pointX = [pointX k1(i).point1(1)]; % storing X cooridinates of all points in an array
    pointXX = [pointXX k1(i).point1(1) k1(i).point2(1)];
  end
  
    avgX=mean(pointX); % WRONG!
   % avgX=(min(pointXX)+max(pointXX))/2; % WRONG!
    for i=1:num1
        if(pointX(i)<avgX)
            lultheta = k1(i).theta+90;
            if(pdist2([jointsX(1),jointsY(1)],[k1(i).point1(1),k1(i).point1(2)])>pdist2([jointsX(1),jointsY(1)],[k1(i).point2(1),k1(i).point2(2)]))
                jointsX = [jointsX k1(i).point1(1)];
                jointsY = [jointsY k1(i).point1(2)];
                lulimbCoor = [k1(i).point2(1) k1(i).point2(2)];% These are the corrdinates of node1*
            else
                jointsX = [jointsX k1(i).point2(1)];
                jointsY = [jointsY k1(i).point2(2)];
                lulimbCoor = [k1(i).point1(1) k1(i).point1(2)];
            end
            break;
        end;
    end;
    ...
    for i=1:num1
        % WRONG! if(abs(k1(i).point1(1)-k1(i).point2(1))>15) % added to ignor small lines
        %if(abs(k1(i).point1(1)-k1(i).point2(1))+abs(k1(i).point1(2)-k1(i).point2(2))>25)
            if(pointX(i)>avgX)
                rultheta = k1(i).theta+90;
                if(pdist2([jointsX(1),jointsY(1)],[k1(i).point1(1),k1(i).point1(2)])>pdist2([jointsX(1),jointsY(1)],[k1(i).point2(1),k1(i).point2(2)]))
                    jointsX = [jointsX k1(i).point1(1)];
                    jointsY = [jointsY k1(i).point1(2)];
                    rulimbCoor = [k1(i).point2(1) k1(i).point2(2)];
                else
                    jointsX = [jointsX k1(i).point2(1)];
                    jointsY = [jointsY k1(i).point2(2)];
                    rulimbCoor = [k1(i).point1(1) k1(i).point1(2)];
                end
                break;
            end;
        %end;
    ...
    for i=1:num2
        if(pointY(i)<avgY)
            ...
            if(pdist2([jointsX(2),jointsY(2)],[k2(i).point1(1),k2(i).point1(2)])>...
               pdist2([jointsX(2),jointsY(2)],[k2(i).point2(1),k2(i).point2(2)]))
           % The joints are node1 and node2 are compared.
                jointsX = [jointsX k2(i).point1(1)];
                jointsY = [jointsY k2(i).point1(2)];
                lllimbCoor = [k2(i).point2(1) k2(i).point2(2)];
            else
                jointsX = [jointsX k2(i).point2(1)];
                jointsY = [jointsY k2(i).point2(2)];
                lllimbCoor = [k2(i).point1(1) k2(i).point1(2)];
            end
            break;
        end;
    end;
    ...
    for i=1:num2
        if(pointY(i)>avgY)
            ...
            if(pdist2([jointsX(3),jointsY(3)],[k2(i).point1(1),k2(i).point1(2)])>pdist2([jointsX(3),jointsY(3)],[k2(i).point2(1),k2(i).point2(2)]))
                jointsX = [jointsX k2(i).point1(1)];
                jointsY = [jointsY k2(i).point1(2)];
                rllimbCoor = [k2(i).point2(1) k2(i).point2(2)];
            else
                jointsX = [jointsX k2(i).point2(1)];
                jointsY = [jointsY k2(i).point2(2)];
                rllimbCoor = [k2(i).point1(1) k2(i).point1(2)];
            end
            break;
        end;
    end;
    ...
    if(jointsY(2)<=lulimbCoor(2) && lultheta<90)
        lultheta = lultheta - 180;
    end
    if(jointsY(3)<rulimbCoor(2))
        rultheta = rultheta - 180;
    end
    if(jointsY(4)<lllimbCoor(2))
        llltheta = llltheta - 180;
    end
    if(jointsY(5)<rllimbCoor(2))
