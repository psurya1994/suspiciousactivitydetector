function [bbox angles joints] = DetectStillVideo_output(obj, ground_truth, pffubfmodel_path,facemodel_path,det_pars,verbose)
...
for i=1:round(obj.NumberOfFrames/30):obj.NumberOfFrames
...
%   end
  %% Additional Code
   ...
   if(~size(bbox,1)==0)
       k = 0;
       for j=1:size(bbox,1)
           box = [bbox(j,1) bbox(j,2) bbox(j,3)-bbox(j,1) bbox(j,4)-bbox(j,2)];
           [T sticks_imgcoor angles joints] = PoseEstimStillImage2(img, 'ubf', round(box(1,1:4)'), fghigh_params, parse_params_Buffy3and4andPascal, [], pm2segms_params, true);
           if (warn == 1) 
               continue; 
           end
           clf;
           k = k + 1;
           similarity(:,k) = classify4(angles,ground_truth);
       end
       %display
       image(img);
       for j=1:k
           if (min(similarity(:,j))<140)
                    showboxes(img,bbox(j,:),'r');
           else
                    showboxes(img,bbox(j,:),'g');
           end
       end
   end
