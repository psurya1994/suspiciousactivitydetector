function [bbox angles joints] = DetectStillImage2(img,pffubfmodel_path,facemodel_path,det_pars,verbose)
...
subplot(2,2,1), image(img);
  ...
  %% Additional Code
   ...
   if(~size(bbox,1)==0)
       for i=1:size(bbox,1)
           box = [bbox(i,1) bbox(i,2) bbox(i,3)-bbox(i,1) bbox(i,4)-bbox(i,2)];
           [T sticks_imgcoor angles joints] = PoseEstimStillImage2(img, 'ubf', round(box(i,1:4)'), fghigh_params, parse_params_Buffy3and4andPascal, [], pm2segms_params, true);
       end
   end
