function [imPose fbox angles joints] = DetectStillImage2(img,pffubfmodel_path,facemodel_path,det_pars,verbose)
...
subplot(2,2,1), image(img);
  ...
  bbox2 = [];
   for i=1:size(bbox,1)
       ...
       area = box(3)*box(4);
       if(area>650 && area<3000 && bbox(i,1)<size(img,2)*.93) % to remove the edge boxes
           bbox2 = [bbox2; box];
       end
   end
   ...
   % Filtering constant illumination boxes
    ...
    for i = 1:size(bbox2,1)
        ...
        if(std2(im(:,:,1))+std2(im(:,:,2))+std2(im(:,:,3))>60)
            bbox3 = [bbox3; bbox2(i,:)];
        end
    end
  ...
  startup;
   ...
   if(~size(bbox4,1)==0)
       ...
       for i=1:size(bbox4,1)
           ...
           k = k + 1;
           similarity2(:,k) = classify4(angles,ground_truth);
           %pause;
       end
   end
   figure(2), imshow(imPose);
   text('units','pixels','position',[100 100],'fontsize',500,'BackgroundColor',[.7 .9 .7],'string',strcat(num2str(suspicious),' suspicious detected!')) 
 
   figure(3), imshow(imPose);
   for j=1:k
           if (min(similarity2(:,j))<140)
                    showboxes(img,fbox(j,:),'r');
           else
                    showboxes(img,fbox(j,:),'g');
           end
       end
