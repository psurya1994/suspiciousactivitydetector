function [imPose fbox angles joints] = DetectStillImage_nothresh(img,pffubfmodel_path,facemodel_path,det_pars,verbose)
...
subplot(2,2,1), image(img);
  ...
  bbox2 = [];
   for i=1:size(bbox,1)
       ...
       area = box(3)*box(4);
       if(area>2000 && area < 10000 && bbox(i,1)<size(img,2)*.93) % to remove the edge boxes
           bbox2 = [bbox2; box];
       end
   end
    ...
    for i = 1:size(bbox3,1)
        if(i==1)
            ...
            for j = 1:size(bbox4,1)
                if( abs(bbox3(i,1)-bbox4(j,1))+abs(bbox3(i,2)-bbox4(j,2)) > 30 )
                    c = c+1;
                end
            end
            if (c == size(bbox4,1))
